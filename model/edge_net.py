...
from model.edge_graph_met import GraphMETNetwork
...
class Net(nn.Module):
    ...
    def forward(self, x_cont, x_cat, edge_index, edge_attr, batch):
        """
        x_cont: [N, n_cont_features]
        x_cat:  [N, n_cat_features]
        edge_index: [2, E]
        edge_attr: [E, n_edge_features]   # new!
        batch: [N] (optional)
        """
        weights = self.graphnet(x_cont, x_cat, edge_index, edge_attr, batch)
        relu_layer = nn.ReLU()
